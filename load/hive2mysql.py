...
def hive2mysql(src_db, src_tb, dst_db, dst_table,dst_columns, etl_mode, res_query, mapper_nums, pre_sql):
    ...
    sqoop_cmd=generate_sqoop_cmd(dst_db, dst_table,dst_columns, etl_mode, export_dir, mapper_nums)
...
def generate_sqoop_cmd( dst_db, dst_table,dst_columns, etl_mode,export_dir,mapper_nums):
    ...
    --mapreduce-job-name %s 
     """ % (dst_table, connect, user_name, password,dst_columns,mapper_nums,export_dir , mapreduce_job_name)
